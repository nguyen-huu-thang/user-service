...
```
Lúc đăng ký:
  Dịch vụ cân bằng tải / placement chọn shard theo tải & dung lượng
    → user.shard_id = "U00000"   (KHÔNG suy ra bằng hash từ identity_id)
  User Service shard "U00000" tạo user record
  (hiện tại: Identity Service gán tạm "U00000" - stub chờ placement service)

Lúc query:
  caller phải biết shard_id để route trực tiếp đến đúng User Service instance
  hoặc → User Locator resolve identifier → shard_id → direct routing
```
